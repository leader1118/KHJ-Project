...
bool	SAcceptor::Run()
{
	...
	while (m_bStarted)
	{
		...
		clientSocket = accept(m_ListenSock, // 접속
...
void	SAcceptor::AddUser(SOCKET socket, SOCKADDR_IN address)
{
	char buf[255] = { 0, };
...
bool	SAcceptor::Set(int iPort, const char* strAddress)
{
	// 윈속 초기화
	int iRet;
	WSADATA wd;
	...
	//////////
	///////////////////////////
	m_ListenSock = socket(AF_INET, SOCK_STREAM, 0); // TCP 서버 
	if (m_ListenSock == INVALID_SOCKET) // INVALID_SOCKET 실패
	{									// 성공 : 새로운 소켓
		return false;
	}

	SOCKADDR_IN serveraddr;
	ZeroMemory(&serveraddr, sizeof(serveraddr)); // 주소를 비운다
	...
	// SO_REUSEADDDR & SO_EXCLUSIVEADDRUSE 둘 중에 하나만 사용한다.
