#include "sCore.h"
LRESULT	sCore::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	return m_ModelCamera.MsgProc(hWnd, msg, wParam, lParam);
}
void sCore::DeleteDeviceResources()
{
	if (m_pDSV)m_pDSV->Release();
	m_Font.DiscardDeviceResources();
	DeleteResources();
}
HRESULT sCore::CreateDeviceResources(UINT width, UINT height)
{
...
void    sCore::SwapModelView()
{
	m_pMainCamera = &m_ModelCamera;
}
void    sCore::SwapDefaultView()
{
	m_pMainCamera = &m_DefaultCamera;
}
...
bool	sCore::GamePreRender()
{
	...
	ApplyDSS(m_pContext, sDxState::g_pDSVStateEnableLessEqual);
	ApplyBS(m_pContext, sDxState::g_pBSAlphaBlend);
	//ApplyRS(m_pContext, xDxState::g_pRSBackCullSolid);
	ApplySS(m_pContext, sDxState::g_pSSWrapLinear);

	if (I_Input.m_KeyState[DIK_P])
		ApplyRS(m_pContext, sDxState::g_pRSWireFrame);
	else
		ApplyRS(m_pContext, sDxState::g_pRSNoneCullSolid);
