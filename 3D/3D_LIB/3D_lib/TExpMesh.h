...
public:
	...
	std::unique_ptr<D3DXVECTOR3[]>        m_pVBPositions;
	std::unique_ptr<D3DXVECTOR3[]>        m_pVBNormals;
	std::unique_ptr<D3DXVECTOR2[]>        m_pVBTexCoords;
public:
	// 2018
	std::vector< T_STR >     m_InfluenceNames;
	std::unordered_map<T_STR, D3DXMATRIX> m_matBindPoseMap;
	TVertexFormat        m_VertexFormat;
	void AddInfluence(T_STR InfluenceName, D3DXMATRIX& mat)
	{
	...
	void WriteColor(BYTE* pDest, const D3DXVECTOR4& Src, DWORD dwDestFormat);
