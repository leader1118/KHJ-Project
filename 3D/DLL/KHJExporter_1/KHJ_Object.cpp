...
void  KHJ_Object::GetTexture(SMtl* DestMtrl, Mtl* pSrcMtl)
{
...
void KHJ_Object::GetObject(MATRIXINDEX* pPoint)
{
...
void KHJ_Object::GetNodeHeader(INode* node, SMesh* Smesh)
{
...
void KHJ_Object::GetNodeTM(INode* node, SMesh* Smesh)
{
	Matrix3 g_matWorld = node->GetNodeTM(m_Interval.Start());
	DumpMatrix3(&g_matWorld, Smesh->m_matWorld);
}

void KHJ_Object::GetMesh(INode* node, SMesh* Smesh)
{
...
Point3 KHJ_Object::GetVertexNormal(Mesh* mesh, int faceNo, RVertex* rv)
{
...
TriObject* KHJ_Object::GetTriObjectFromNode(INode* node, TimeValue t, int &deletelt)
{
	deletelt = FALSE;
