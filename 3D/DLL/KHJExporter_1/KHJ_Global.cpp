...
int KHJ_Global::GetIndex(const TCHAR* strNodeName)
{
	return m_MatrixMap.GetID(strNodeName);
}

bool KHJ_Global::CheckFile(Interface* pMax)
{
...
bool KHJ_Global::Initialize(Interface* pMax)
{
	m_p3dsMax = pMax;
...
void KHJ_Global::PreProcess(INode* pNode)
{
...
void KHJ_Global::AddObject(INode* pNode)
{
	// 오브젝 트 추가
...
void KHJ_Global::AddMtl(INode* pNode)
{
	// 매터리얼 추가 (중복제거)
...
int KHJ_Global::GetMtlref(Mtl* pMtl) // 매터리얼 레퍼런스 정보
{
...
TCHAR* KHJ_Global::FixupName(MSTR name)
{
...
void KHJ_Global::DumpMatrix3(Matrix3* m, D3D_MATRIX& mat)
{
	Point3 row;
	row = m->GetRow(0);
...
void KHJ_Global::DumpPoint3(Point3& pDest, Point3& pSrc)
{
	pDest.x = pSrc.x;
	pDest.y = pSrc.z;
	pDest.z = pSrc.y;
...
bool KHJ_Global::TMNegParity(Matrix3 &m)
{
	return (DotProd(CrossProd(m.GetRow(0), m.GetRow(1)),
		m.GetRow(2)) < 0.0f) ? 1 : 0;
}

bool KHJ_Global::EqualPoint2(Point2 p1, Point2 p2)
{
...
bool KHJ_Global::EqualPoint3(Point3 p1, Point3 p2)
{
	if (fabs(p1.x - p2.x) > ALMOST_ZERO)
		return false;
	if (fabs(p1.y - p2.y) > ALMOST_ZERO)
...
bool KHJ_Global::EqualPoint4(Point4 p1, Point4 p2)
{
	if (fabs(p1.x - p2.x) > ALMOST_ZERO)
		return false;
	if (fabs(p1.y - p2.y) > ALMOST_ZERO)
