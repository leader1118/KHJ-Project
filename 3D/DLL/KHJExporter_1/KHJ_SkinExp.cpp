#include "KHJ_SkinExp.h"
#include "KHJ_Writer.h"


bool KHJ_SkinExp::Init()
{
	return true;
}
void KHJ_SkinExp::ExportPhysiqueData(INode* node, Modifier *phyMod)
{
...
void  KHJ_SkinExp::ExportSkinData(INode* node, Modifier *skinMod)
{
...
void KHJ_SkinExp::SetBipedInfo(INode* node)
{
...
// 노드에 대한 정보 획득
bool KHJ_SkinExp::GetNodeInfo(INode* node, TimeValue t)
{
	// 랜더가능한 데이터 정보 버퍼 저장.
	BOOL needDel;
...
void KHJ_SkinExp::SetVertexBiped(INode* node, Face* face, int v0, int v1, int v2, SSkinTri* pTri)
{
	int iNumWeight = 1;
	if (m_BipedList.size())
	{
...
void KHJ_SkinExp::SetTriangle(INode* node, Mesh* mesh, SSkinMesh* pSkinMesh, SSkinMesh*pSubMesh, int iMtrl)
{
...
void KHJ_SkinExp::GenerateGroup(INode* node, Mesh *mesh, SSkinMesh* pSkinMesh)
{
	Mtl* mtl = node->GetMtl();
...
void KHJ_SkinExp::LoadMaterial(SSkinMesh*	pMesh, Mtl* mtl)
{
	if (mtl)
	{
		pMesh->m_iNumTex = 0;
...
// 피직크 검색
Modifier*	KHJ_SkinExp::FindModifier(INode *nodePtr, Class_ID classID)
{
...
int KHJ_SkinExp::GetMapID(Class_ID cid, int subNo)
{
	int iResultIndex = 1;
	if (cid == Class_ID(0, 0))
	{
...
int KHJ_SkinExp::IsEqulVertexList(vector<PNCT5_VERTEX>&  VertexArray, PNCT5_VERTEX& Vertex)
{
...
bool KHJ_SkinExp::ExportMesh(FILE* fp, SSkinMesh* pMesh)
{
