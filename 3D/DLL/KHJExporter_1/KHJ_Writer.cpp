...
bool KHJ_Writer::Init(TSTR strFileName, Interface* pMax)
{
	m_strFileName = strFileName;
	return Initialize(pMax);
}
TCHAR* KHJ_Writer::SaveFileDlg(TCHAR* szExt, TCHAR* szTitle)
{
	OPENFILENAME    ofn;
...
bool KHJ_Writer::Release()
{
	m_pMtlList.clear();
	m_MatrixMap.Release();
	return true;
}
void KHJ_Writer::ExpScene()
{
	struct tm *newtime;
	time_t aclock;
...
void KHJ_Writer::ExpTexture(vector<STexMap>& TexMap)
{
...
int KHJ_Writer::IsEqulVertexList(vector<PNCT_VERTEX>&  VertexArray, PNCT_VERTEX& Vertex)
{
...
int KHJ_Writer::SetUniqueBuffer(SMesh* pMesh, int iMtrl, int iStartTri, vector <PNCT_VERTEX>& VertexArray, vector<int>& IndexArray)
{
...
int KHJ_Writer::ExpMesh(FILE* fp, SMesh* pMesh, int iMtrl, int iAddCount)
{
...
void KHJ_Writer::ExpAnimation(SMesh* pMesh)
{
