...
struct MATRIXINDEX
{
	...
	int m_iIndex;
	T_STR m_szName;
	vector<P3NCVERTEX> m_VertexList;
	Matrix3 m_NodeTM;
	Matrix3 m_InvNodeTM;
	INode* m_pINode;
	Object* m_pObj;
	...
	void Add(int iIndex, TSTR szName)
	{
		m_pObj = NULL;
		m_pControl = NULL;
		m_iIndex = iIndex;
	...
	void SetNode(INode* pNode, int iTick, MATRIXINDEX* pParentIndex = 0)
	{
		...
		if (m_pObj&&m_pObj->ClassID() == Class_ID(BONE_CLASS_ID, 0))
		{
			m_ClassType = CLASS_BONE;
		}
		if (m_pObj&&m_pObj->ClassID() == Class_ID(DUMMY_CLASS_ID, 0))
		{
			m_ClassType = CLASS_DUMMY;
		}
		if (m_pControl->ClassID() == BIPSLAVE_CONTROL_CLASS_ID ||
			m_pControl->ClassID() == BIPBODY_CONTROL_CLASS_ID)
		{
...
public:
	...
	SScene		 m_Scene; // 씬정보 저장
	STemplateMap<MATRIXINDEX> m_MatrixMap;
...
public:
	...
	int			 GetMtlRef(Mtl* pMtl);// pMtl 이 저장된 인덱스를 반환한다.
