...
{
	bool operator()(STri& rpStart, STri& rpEnd)
	{
...
void S_Writer::matExport()
{
	m_strFileName = I_Writer.SaveFileDlg(_T("mat"), _T("SMatrixExport"));
	FILE *fp;
	...
	SScene scene = m_Scene;
	scene.iNumMesh = m_MatrixMap.Count();
	fwrite(&scene, sizeof(SScene), 1, fp);
	...
	for (DWORD iBiped = 0; iBiped < m_MatrixMap.Count(); iBiped++)
	{
		...
		else if (pPoint->m_ClassType == CLASS_BONE || pPoint->m_ClassType == CLASS_DUMMY)
		{
			...
			vVertex[1].Px = bbox.pmax.x;
			vVertex[1].Py = bbox.pmax.z;
			vVertex[1].pz = bbox.pmax.y;
...
bool S_Writer::Init(TSTR strFileName, Interface* pMax)
{
	m_strFileName = strFileName;
	return Initialize(pMax);
}

TCHAR* S_Writer::SaveFileDlg(TCHAR* szExt, TCHAR* szTitle)
{
	OPENFILENAME ofn;
...
bool S_Writer::KHJExport()
{
	GetMaterial();
	UpdateObject();

	TCHAR szFile[256] = { 0, };
	_tcscpy(szFile, I_Writer.SaveFileDlg(_T("KHJ"), _T("KHJExpoter")));

	m_pStream = _tfopen(szFile, _T("wt"));
	if (!m_pStream)
	{
		return 0;
	}
...
void S_Writer::ExpTexture(vector<STexMap>& TexMap)
{
...
int S_Writer::IsEqulVertexList(vector<PNCT_VERTEX>& VertexArray, PNCT_VERTEX& Vertex)
{
...
int S_Writer::SetUniqueBuffer(SMesh* pMesh, int iMtrl, int iStartTri, vector <PNCT_VERTEX>& VertexArray, vector<int>& IndexArray)
{
...
int S_Writer::ExpMesh(FILE* fp, SMesh* pMesh, int iMtrl, int iAddCount)
{
...
void S_Writer::ExpAnimation(SMesh* pMesh)
{
