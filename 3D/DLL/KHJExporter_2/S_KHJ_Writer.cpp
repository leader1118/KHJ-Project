...
bool S_KHJ_Writer::Init(TSTR strFileName, Interface* pMax)
{
	m_strFileName = strFileName;
	return Initialize(pMax);
}
TCHAR* S_KHJ_Writer::SaveFileDlg(TCHAR* szExt, TCHAR* szTitle)
{
	OPENFILENAME    ofn;
...
bool S_KHJ_Writer::KHJExport()
{
	...
	TCHAR  szFile[256] = { 0, };
	_tcscpy(szFile, I_Writer.SaveFileDlg(_T("tbs"), _T("TbsExport")));
...
bool S_KHJ_Writer::Release()
{
	m_pMtlList.clear();
	m_MatrixMap.Release();
	return true;
}
void S_KHJ_Writer::ExpScene()
{
	struct tm *newtime;
	time_t aclock;
...
void S_KHJ_Writer::ExpTexture(vector<STexMap>& TexMap)
{
...
int S_KHJ_Writer::IsEqulVertexList(vector<PNCT_VERTEX>&  VertexArray, PNCT_VERTEX& Vertex)
{
...
int S_KHJ_Writer::SetUniqueBuffer(SMesh* pMesh, int iMtrl, int iStartTri, vector <PNCT_VERTEX>& VertexArray, vector<int>& IndexArray)
{
...
int S_KHJ_Writer::ExpMesh(FILE* fp, SMesh* pMesh, int iMtrl, int iAddCount)
{
...
void S_KHJ_Writer::ExpAnimation(SMesh* pMesh)
{
...
S_KHJ_Writer::S_KHJ_Writer(void)
{
}
