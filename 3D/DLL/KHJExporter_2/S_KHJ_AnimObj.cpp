...
void S_KHJ_AnimObj::GetAnimKeys(INode* pNode, SMesh* pMesh)
{
	BOOL bPosAnim;
	BOOL bRotAnim;
	BOOL bScaleAnim;
...
bool S_KHJ_AnimObj::GetDecompAffine(TimeValue t, INode* pNode, AffineParts* ap, Point3* rotAxis, float* rotAngle)
{
...
BOOL S_KHJ_AnimObj::CheckForAnimation(INode* pNode, BOOL& bPos, BOOL& bRot, BOOL& bScale)
{
	AffineParts ap;
...
void S_KHJ_AnimObj::DumpPosSample(INode* pNode, SMesh* pMesh)
{
...
void S_KHJ_AnimObj::DumpRotSample(INode* pNode, SMesh* pMesh)
{
	TimeValue start = m_Interval.Start();
	TimeValue end = m_Interval.End();
...
void S_KHJ_AnimObj::DumpScaleSample(INode* pNode, SMesh* pMesh)
{
	TimeValue start = m_Interval.Start();
	TimeValue end = m_Interval.End();
...
void S_KHJ_AnimObj::DumpFloatKeys(Control* cont, SMesh* pMesh)
{
	if (!cont) return;
	int i;
...
S_KHJ_AnimObj::S_KHJ_AnimObj(void)
{
	m_iDeltaTick = 160;
}
