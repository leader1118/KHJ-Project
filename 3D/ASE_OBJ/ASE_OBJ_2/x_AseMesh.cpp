...
void   x_AseMesh::LoadMaterial()
{
	...
	for (int iCnt = 0; iCnt < iCount; iCnt++)
	{
		...
		int iSubMtrl=0;
		if (FineString(_T("*MATERIAL_CLASS")) == false) return;
		TCHAR szClass[256] = { 0, };
		_stscanf(m_pBuffer, _T("%s%s"), m_pString, &szClass);
		if (!_tcsicmp(szClass, _T("\"Multi/Sub-Object\"")))
		{
			
			if (FineString(_T("*NUMSUBMTLS")) == false) return;
			_stscanf(m_pBuffer, _T("%s%d"), m_pString, &iSubMtrl);
		}
		if (iSubMtrl > 0)
		{
			for (int iSub = 0; iSub < iSubMtrl; iSub++)
			{
				...
				_stscanf(m_pBuffer, _T("%s%d"), m_pString, &tSubMap.iType);
				_fgetts(m_pBuffer, 256, m_pStream);
				_fgetts(m_pBuffer, 256, m_pStream);
				...
				ext[4] = 0;
				tSubMap.name += ext;
				xSubMat.m_mapList.push_back(tSubMap);
				xMtrl.m_SubMaterial.push_back(xSubMat);
			}
		}
		else
		{
			...
			ext[4] = 0;
			tMap.name += ext;
			xMtrl.m_mapList.push_back(tMap);
		}
		m_MaterialList.push_back(xMtrl);
	}
}
